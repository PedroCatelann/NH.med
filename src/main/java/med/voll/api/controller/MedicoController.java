...
public class MedicoController {
    ...
    public ResponseEntity cadastrar(@RequestBody @Valid DadosCadastroMedico dadosCadastroMedico, UriComponentsBuilder uriComponentsBuilder) {
        var medico = repository.save(new Medico(dadosCadastroMedico));
        var uri = uriComponentsBuilder.path("/medicos/{id}").buildAndExpand(medico.getId()).toUri();
        return ResponseEntity.created(uri).body(new DadosDetalhamentoMedico(medico));
    }
    @GetMapping
    public ResponseEntity<Page<DadosListaMedico>> listarMedico(@PageableDefault(size = 10, sort = {"nome"}) Pageable paginacao) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity detalhat(@PathVariable Long id) {
        var medico = repository.getReferenceById(id);
        return ResponseEntity.ok(new DadosDetalhamentoMedico(medico));
    }
}
